...
This script verifies **Post Negotiation Analysis Only** (debrief) vs **Live Call** modes, ensuring strict separation of concerns and privacy standards.
...
## Test 1: Debrief Mode (Post Negotiation Analysis Only) Policy Check
**Objective:** Verify that NO live advice is generated in debrief mode, and that the system only acts as a passive recorder until the session ends.

### 1. Pre-Flight Setup
1. Launch the App.
2. **Verify:** "NEGOTIATION PRE-FLIGHT" screen appears.
3. **Action:** Select Scenario: **Vendor**.
4. **Action:** Select preset **Post Negotiation Analysis Only**.
...
2. **Observation Window (60 seconds):**
   ...
   - The UI should show "🎤 LISTENING..." and audio meters moving.
...
## Test 2: Live Call Mode (Opt-In)
**Objective:** Verify that selecting the Live Call preset enables real-time tactical signals.

### 1. Pre-Flight Setup
1. Click "Back" or Restart App.
2. **Action:** Select Scenario: **Renewal**.
3. **Action:** Select preset **Live Call**.
...
2. **Observation (Wait ~15-20 seconds):**
   - **Verify:** A signal alert appears (e.g., "⚠️ URGENCY DETECTED").
   - **Verify:** It offers 2-3 brief options (e.g., "Consider asking for an extension", "One option is to verify the deadline").
